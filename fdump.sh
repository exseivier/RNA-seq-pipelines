...
echo -n "Work name?: "; read NAME; if [ "$NAME" == "" ]; then NAME="Unknown"; fi
...
#<<MAIN_PRPGRAM
	echo "
fastq-dump2.7.0 --outdir $PRWD/$READS \
--gzip \
--split-files \
$OPTIONAL_PARAMETERS \
${arrLINE[0]}
files=$(ls $READS/${arrLINE[0]%.sra}_*.* | tr "\n" "|" | sed 's/sra\///g')
echo "${arrLINE[0]%.sra}.sam|${files}" >> $COMMANDS/$TRIMMING_COMMANDS" | qsub -N $NAME -l nodes=$NODES:ppn=$PROCESSOR_PER_NODE,vmem=${VIRTUAL_MEMORY}gb,mem=${MEMORY}gb -V -q $QUEUE
#MAIN_PRPGRAM
